...
##Install DESeq2, Mara added mygene
...
#Load Libraries added virdisisLite and apeglm
library(DESeq2); library(ggplot2); library(EnhancedVolcano);library(apeglm); library(GOplot); library(mygene); library(tidyverse); library(pheatmap); library(reshape2); library(viridis)

library(viridisLite); library(reshape2); library(viridis); 

# Set working directory to source file location

#Choose directory with htseq-count data
directory<-("/Users/mch284/Documents/Armbruster Lab/Auto_biting/fr_counts_dir")
...
sampleConditions <- substr(sampleFiles, 1, 1)#to get conditions I'm pulling the first letter, which is either N (non-biting) or B (biting)
...
#Relevel the condition for what to compare to.  (Default is first condition alphabetically. Since we want to compare to non-biters, set ref="N")
...
# Note: have to match order of N and B...B_vs_N if setting dds ref =N  Also had to install apglm

resultsNames(dds)
res_LFC <- lfcShrink(dds, coef="condition_B_vs_N", type="apeglm")
res_LFC
...
# write.csv(resOrdered, 
          file="/Users/mch284/Documents/Armbruster Lab/Auto_biting/resOrdered_BVN.csv", row.names = T)
...
#try padj for y instead of -pvalue and set pcutoff to .05 instead of 10e-6, ylim changed to 15 from 25
...
#Mara note-- load magrittr: library(magrittr)
...
write.csv(dplyr::select(sig_res, gene, log2FoldChange, padj), 
          file="/Users/mch284/Documents/Armbruster Lab/Auto_biting/sig_res_BVN.csv", row.names = F)

# Write out just the gene names for later analysis in KEGG
write.table(sig_res %>% dplyr::select(gene), 
            file="/Users/mch284/Documents/Armbruster Lab/Auto_biting/BvN_DEGids.txt", col.names = F, row.names = F, quote = F)
...
#to get gene names
res_print <- as.data.frame(sig_res)


head(res_print)

# Find gene names with mygene package
res_print<-read.csv("day_BvN_LFCshrink_padj.csv", header=TRUE)
head(res_print)
library(mygene)

dat <- queryMany(res_print$gene, scopes="symbol", fields=c("name"))

res_merged <- merge(res_print, dat, by.x="gene", by.y="query")
view(res_merged)
keep_cols <- c("gene", "name", "log2FoldChange", "padj")
head(keep_cols)

# Write out a csv with these data
write.csv(res_merged[keep_cols],
          file="/Users/mch284/Documents/Armbruster Lab/Auto_biting/BvN_day_sig_DESeq_results_with_genenames.csv", row.names = F)
